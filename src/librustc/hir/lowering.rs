...
impl<'a> LoweringContext<'a> {
    ...
    fn expect_full_def_per_ns(&mut self, id: NodeId) -> PerNS<Def> {
        self.resolver.get_all_resolutions(id).map(|res| {
            res.map_or(Def::Err, |pr| {
                if pr.unresolved_segments() != 0 {
                    bug!("path not fully resolved: {:?}", pr);
                }
                pr.base_def()
    ...
    fn lower_path_per_ns(&mut self, id: NodeId, p: &Path, param_mode: ParamMode) -> PerNS<hir::Path> {
        self.expect_full_def_per_ns(id).map(|def| {
            hir::Path {
                def,
                segments: p.segments
                    .iter()
        ...
        match tree.kind {
            UseTreeKind::Simple(rename) => {
                *name = tree.ident().name;
                ...
                let mut paths = self.lower_path_per_ns(id, &path, ParamMode::Explicit)
                                    .into_iter()
                                    .collect::<Vec<_>>();

                if paths.iter().all(|p| p.def == Def::Err) {
                    // if all the namespaces came up with errors, then there's nothing to point at,
                    // just keep one
                    paths.resize_with(1, || unreachable!());
                } else {
                    // if there was something usable, then just trash the ones that weren't
                    paths.retain(|p| p.def != Def::Err);
                }

                hir::ItemUse(paths.into(), hir::UseKind::Single)
            }
            UseTreeKind::Glob => {
                ...
                hir::ItemUse(vec![path].into(), hir::UseKind::Glob)
            }
            UseTreeKind::Nested(ref trees) => {
                ...
                let path = self.lower_path(id, &prefix, ParamMode::Explicit);
                *vis = hir::Inherited;
                hir::ItemUse(vec![path].into(), hir::UseKind::ListStem)
            }
        }
    }
